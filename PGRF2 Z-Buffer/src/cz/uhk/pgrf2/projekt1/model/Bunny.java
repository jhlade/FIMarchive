 ...
 *
 */
public class Bunny implements ISolid, ISurface {
	...
	public Bunny() {
		try {
			TriParser sphere = new TriParser(this.getClass().getResourceAsStream("/cz/uhk/pgrf2/projekt1/data/bunny.tri"));
	...
	public List<Integer> colors() {
		if (indices.size() % 3 != 0) System.out.println("Buch! Creepři dorazili.");
		
		if (colors.size() == 0) {
			for (int i = 0; i < indices.size()/3; i++) {
				colorsAsList.add(0xffc0c0c0);
			}
		}
